...
if (set_data) then
-- 306. Int32 ar1.ChanNAdcConfig(UInt16 Tx0En, UInt16 Tx1En, UInt16 Tx2En, UInt16 Rx0En, UInt16 Rx1En, UInt16 Rx2En, UInt32 Rx3En, Int32 BitsVal, UInt32 FmtVal, UInt32 IQSwap)                                                                                               
...
-- 309. Int32 ar1.LPModConfig(Int32 AnaChan, Int32 LpAdcMod)                        
...
elseif((partId == 1243) or (partId == 1443)) then
    ...
    -- if(ar1.ProfileConfig(0, actual_start, 100, start_time, 60, 0, 0, 0, 0, 0, 0, slope, 0, num_samples, sample_rate*1000, 0, 0, 30) == 0) then
    if(ar1.ProfileConfig(0, actual_start, 100, start_time, 60, 0, 0, 0, 0, 0, 0, slope, 0, num_samples, sample_rate*1000, 0, 0, RX_GAIN) == 0) then
        WriteToLog("ProfileConfig Success\n", "green")
    else
        WriteToLog("ProfileConfig failure\n", "red")
    end
...
-- 1. Int32 ar1.ChirpConfig(UInt16 chirpStartIdx, UInt16 chirpEndIdx, UInt16 profileId, Single startFreqVar, Single freqSlopeVar, Single idleTimeVar, Single adcStartTimeVar, UInt16 tx0Enable, UInt16 tx1Enable, UInt16 tx2Enable)                                                                                                                                                   
-- -  Chirp configuration API which defines which profile is to be used for each chirp in a frame
...
-- 312. Int32 ar1.FrameConfig(UInt16 chirpStartIdx, UInt16 chirpEndIdx, UInt16 frameCount, UInt16 loopCount, Single periodicity, Single triggerDelay, Byte numDummyChirp, UInt16 TriggerSelect)                                                                                                               
-- -  Frame Configuration API defines Frame formation which has sequence of chirps to be transmitted subsequently
-- periodicity likely in ms
periodicity = 3 --Dataset: 125
--1000 = speed1 -- is in milliseconds -- decrease this if you increase the robot speed (by the same multiplicitive factor) used to be 125 Tara changed it to 100 on Aug 14
-- Record 8 chirps = 65536 bytes in cf.json
num_frames = 4000--Circle: 5632 --Dataset: 110
-- Dataset cf.json bytes=  Dataset: 901120
if (ar1.FrameConfig(0, 0, num_frames, 1, periodicity, 0, 0, 1) == 0) then
...
if (ar1.CaptureCardConfig_EthInit("192.168.33.42", "192.168.33.180", "12:34:56:78:90:12", 4096, 4098) == 0) then
...
-- ar1.CaptureCardConfig_StartRecord(data_path, 1)
-- RSTD.Sleep(1000)
