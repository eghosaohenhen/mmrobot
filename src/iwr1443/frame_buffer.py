...
Modified from: https://github.com/ConnectedSystemsLab/xwr_raw_ros/blob/main/src/xwr_raw/frame_buffer.py
"""
...
class FrameBuffer:
    ...
    def add_msg(self, seqn, msg):
        ...
        if self.curr_idx + msg.size > self.buffer.size:
            ...
            self.buffer[: msg.size - (self.buffer.size - self.curr_idx)] = msg[
                (self.buffer.size - self.curr_idx) :
            ]
        else:
        ...
        if old_frame_idx != new_frame_idx:
            return (
                self.buffer[
                    old_frame_idx
                    * self.frame_size : (old_frame_idx + 1)
                    * self.frame_size
                ].view(np.int16),
                True,
            )
